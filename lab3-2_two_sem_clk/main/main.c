...
/* Datas */
int IncTable[TABLE_SIZE];
int DecTable[TABLE_SIZE];
...
void app_main(void) {
	...
	/* Init Table */
	memset(IncTable, 0, TABLE_SIZE*sizeof(int));
	memset(DecTable, 0, TABLE_SIZE*sizeof(int));
	...
	for(;;){
		...
		if(ActivationNumber==0){
			...
			for(int i=0;i<TABLE_SIZE;i++){
				IncTable[i]=IncTable[i]+constNumber;
			//	DISPLAY("INCREMENTE");
...
void vTaskDecTable(void *pvParameters) {
	for(;;){
		...
		for(int i=0;i<TABLE_SIZE;i++){
			DecTable[i]=DecTable[i]-constNumber;
